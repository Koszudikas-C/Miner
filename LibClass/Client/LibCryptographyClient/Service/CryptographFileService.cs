...
namespace LibCryptographyClient.Service;

public class CryptographFileService : ICryptographFile
{
    public byte[] SaveFile(ConfigCryptograph configCryptograph)
    {
        if (IsFileEncrypted(configCryptograph.FilePath!, configCryptograph.HeaderSignature))
            throw new CryptographicException("File is already encrypted");

        var encryptedData = Encrypt(configCryptograph.GetEncryptKey()!,
            configCryptograph.GetDataBytes(),
            configCryptograph.HeaderSignature);

        var hmac = EncryptHmac(configCryptograph.GetHmacKey()!, encryptedData);

        var result = encryptedData.Concat(hmac).ToArray();
        File.WriteAllBytes(configCryptograph.FilePath!, result);

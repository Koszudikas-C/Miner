...
using System.Text;
using LibSocks5Remote.Entities.Enum;
using LibSocks5Remote.Entities;
using LibSocks5Remote.Interface;

namespace LibSocks5Remote.Service;
...
public class Socks5Service : ISocks5
{
    ...
    private static async Task SelectAuth(Socket socket, Socks5Options options, CancellationToken cts = default)
    {
        ...
        if (read != 2)
            throw new ArgumentNullException($"Failed to select an authentication" +
                                            $" method, the server sent {read} bytes.", nameof(read));
    ...
    private static byte GetDestAddressType(string host)
    {
        ...
        switch (ipAddr.AddressFamily)
        {
            ...
            default:
                throw new Exception(
                    $"The host address {host} of type '{Enum.GetName(typeof(AddressFamily), ipAddr.AddressFamily)}' is not a supported address type.\n" +
